...
def getSubDirs(mainDirPath):
    ...
    onlyClassifierPaths = []
    for subdir_path in list_subfolders_with_paths:
        if subdir_path.endswith("common_plots") or subdir_path.endswith("common_docs"):
            continue
        else:
            onlyClassifierPaths.append(subdir_path)

    # return list_subfolders_with_paths
    return onlyClassifierPaths
...
def get_immediate_subdirectories(mainDir):
    imm_subdirs = [name for name in os.listdir(mainDir) if os.path.isdir(os.path.join(mainDir, name))]
    onlyClassifierPaths = []
    for subdir_path in imm_subdirs:
        if subdir_path.endswith("common_plots") or subdir_path.endswith("common_docs"):
            continue
        else:
            onlyClassifierPaths.append(subdir_path)
...
if __name__ == "__main__":
    ...
    print(f"\nThe selected directory:\n{main_dir_path}\n\nSubdirectories in the selected main directory:\n{dataset_dir_paths}\n")

    dataset_names = get_immediate_subdirectories(mainDir=main_dir_path)
    print(f"The list of immediate subdirectory names in the selected main directory:\n{dataset_names}\n")
    ...
    for drpID, ds_res_path in enumerate(dataset_dir_paths):
        ...
        for cdpID, cdp in enumerate(class_dir_paths):
            ...
            for rdpID, rdp in enumerate(regressor_dir_paths):
                ...
                if regressor_name == "Lasso Regression":
                    regressor_name = 'LasReg'
                elif regressor_name == "Linear Regression":
                    regressor_name = 'LinReg'
                elif regressor_name == "Logistic Regression":
                    regressor_name = 'LogReg'
                elif regressor_name == 'Ridge Regression':
                    regressor_name = 'RidReg'                
