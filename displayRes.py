...
def display_train_test_time_performance(df, fn):
    ...
    # time_performance_metrics = [ 'Training Time', 'Test Time-All Records', 'Test Time-Per Record']
    time_performance_metrics = ['MethodShortName', 'Training Time', 'Test Time-All Records']
    ...
    ax.set_title(f"The Train & Test Metrics of the Systems, Trained on \n the Full and Subsets of the '{dataset_name.upper()}' dataset", \
                 fontsize=29, fontweight="bold")
    ax.set_xlabel(f"Systems by Classifiers, Kernel Regressors and Feature Selectors", \
                  fontsize=25, fontweight="bold")
    ax.set_ylabel("Record Processing Time (milliseconds)", fontsize=25, fontweight="bold")
    ...
    # y_test_all = df['Test Time-All Records']
    ...
    plt.savefig(f"./TimePerformMetrics-{dataset_name}.png", dpi = 100)
    plt.show()

def display_entire_metrics(df, fn, eval_metrics):
    dataset_name = fn
    # species = ("Adelie", "Chinstrap", "Gentoo")

    # time_performance_metrics = [ 'Training Time', 'Test Time-All Records', 'Test Time-Per Record']
    eval_metric_Entire = ['MethodShortName', 'Precision(Benign)', 'Precision(Malignant)', 'Re-Call(Benign)', 'Re-Call(Malignant)', 'F1-Score(Benign)', 'F1-Score(Malignant)']
    eval_metric_Precision = ['MethodShortName', 'Precision(Benign)', 'Precision(Malignant)']
    eval_metric_ReCall = ['MethodShortName', 'Re-Call(Benign)', 'Re-Call(Malignant)']
    eval_metric_F1Score = ['MethodShortName', 'F1-Score(Benign)', 'F1-Score(Malignant)']

    # Bar color and thickness
    colors = ["c", "mediumblue", "yellow", "orange", "springgreen", "green"]
    width = 0.37  # the width of the bars

    accuracy_metrics = eval_metric_Entire
    if eval_metrics == "Precision":
        accuracy_metrics = eval_metric_Precision
        colors = ["springgreen", "red"]
    elif eval_metrics == "ReCall":
        accuracy_metrics = eval_metric_ReCall
        colors = ["lime", "magenta"]
    elif eval_metrics == "F1Score":
        accuracy_metrics = eval_metric_F1Score
        colors = ["green", "deeppink"]
    else:
        accuracy_metrics = eval_metric_Entire
        width = 0.123
    ...
    for acmet in accuracy_metrics[1:]:
    ...
    for attribute, measurement in acc_metrics_with_values.items():
        attribute = re.sub(r"[\n\t\s]*", "", attribute)
        offset = width * multiplier
        rects = ax.bar(x + offset, measurement, width, label=attribute, color=colors[color_ID], alpha=0.61)
        # ax.bar_label(rects, padding=3)
        multiplier += 1
        color_ID += 1
    ...
    ax.set_title(f"The {eval_metrics} Metrics of the Systems, Trained on the Full and Subsets of the '{dataset_name.upper()}' dataset", \
                 fontsize=24, fontweight="bold")
    ax.set_xlabel(f"Systems by the Classifiers and Regression Model-based Feature Selectors", \
                  fontsize=22, fontweight="bold")
    ax.set_ylabel("Systems Metrics by Percentage", fontsize=22, fontweight="bold")
    ...
    # ax.set_ylabel('Length (mm)')
    # ax.set_title('Penguin attributes by species')
    ax.set_xticks(x + width, acc_metric_types, rotation=90, ha='right', fontsize=15, fontweight="bold")
    legend_properties = {'weight':'bold', 'size':16}
    # plt.legend(title="$\\bf{Features\ Selected\ with:}$", \
    #            title_fontsize=21, prop=legend_properties)
    ax.legend(loc='upper right', ncols=len(accuracy_metrics)-2, prop=legend_properties, fancybox=True, framealpha=0.51)
    plt.yticks(ticks=np.arange(0.0, 1.05, 0.05), fontsize=15, fontweight="bold")
    ax.set_ylim(0, 1.1)

    plt.savefig(f"./{eval_metrics}-{dataset_name}.png", dpi = 100)
    plt.show()
...
def display_entire_results(df, fn):
    ...
    # time_performance_metrics = [ 'Training Time', 'Test Time-All Records', 'Test Time-Per Record']
    accuracy_metrics = ['MethodShortName', 'Precision(Benign)', 'Precision(Malignant)', 'Re-Call(Benign)', 'Re-Call(Malignant)', 'F1-Score(Benign)', 'F1-Score(Malignant)', 'Accuracy']
    
    df_temp = df[accuracy_metrics]
    print(f"df cols:\n{df.columns}\nJust accuracy metrics:\n{accuracy_metrics}\n")

    # plot grouped bar chart
    precision_cols = ['MethodShortName', 'Precision(Benign)', 'Precision(Malignant)']
    re_call_cols = ['MethodShortName', 'Re-Call(Benign)', 'Re-Call(Malignant)']
    f1_score_cols = ['MethodShortName', 'F1-Score(Benign)', 'F1-Score(Malignant)']
    ...
    plt.title(f"Accuracy of the Systems, Trained on the Full and Subsets of the '{dataset_name}' dataset", fontsize=29, fontweight="bold")
    plt.xlabel(f"Systems by the Classifiers and Regression Model-based Feature Selectors", fontsize=23, fontweight="bold")
    plt.ylabel("Systems Accuracy", fontsize=23, fontweight="bold")
    plt.show()
...
def display_entire_classification_acc_metrics(df, fn):
    ...
    plt.figure(figsize=(35,15), layout="constrained")
    plt.xticks(rotation=45, ha='right', fontsize=17, fontweight="bold")
    plt.yticks(ticks=np.arange(0.0, 1.03, 0.05), fontsize=15, fontweight="bold")
    
    plt.ylim(0.0, 1.01)
    
    plt.title(f"Accuracy of the Systems, Trained on the Full and Subsets of the '{dataset_name.upper()}' dataset", fontsize=29, fontweight="bold")
    plt.xlabel(f"Systems by the Classifiers and Regression Model-based Feature Selectors", fontsize=23, fontweight="bold")
    plt.ylabel("Systems Accuracy", fontsize=23, fontweight="bold")
    ...
    # plot grouped bar chart
    precision_cols = ['Precision(Benign)', 'Precision(Malignant)']
    re_call_cols = ['Re-Call(Benign)', 'Re-Call(Malignant)']
    f1_score_cols = ['F1-Score(Benign)', 'F1-Score(Malignant)']
...
def addlabels(x,y,ha,va,rotation):
    for i in range(len(x)):
        plt.text(x=i, y=y[i]-0.081, s=f"{y[i]}", ha=ha, va=va, rotation=rotation, fontdict={"size":14, "weight":"bold"})

# Display classifier accuracy by a trained dataset
def display_signle_classification_acc_metrics(df, fn):
    dataset_name = fn

    plt.figure(figsize=(23,9), layout="constrained")
    plt.xticks(rotation=90, ha='right', fontsize=16, fontweight="bold")
    plt.yticks(ticks=np.arange(0.0, 1.0, 0.05), fontsize=17, fontweight="bold")
    
    plt.ylim(0.0, 1.01)
    
    plt.title(f"Accuracy of the Systems, Trained on the Full and Subsets of the '{dataset_name.upper()}' dataset",\
               fontsize=27, fontweight="bold")
    plt.xlabel(f"Systems by the Classifiers and Regression Model-based Feature Selectors", fontsize=25, fontweight="bold")
    plt.ylabel("System Accuracy", fontsize=23, fontweight="bold")
    ...
    y = df['Accuracy']
    addlabels(x=x, y=y, ha="center", va="center", rotation=90)
    ...
    bar_color, plot_color, marker = determine_acc_bar_and_plot_colors(dataset_name)
    plt.plot(x, y+0.005, color=plot_color, linewidth=3, linestyle="-", marker=marker, markersize=17, alpha=.61)
    plt.bar(height=y, x=x, color=bar_color, width=0.7, alpha=0.51)    

    # plt.grid(axis = 'y')
    plt.savefig(f"./Accuracy-{dataset_name}.png", dpi = 100)
    plt.show()

def display_training_time_metrics(df, fn):
    dataset_name = fn

    plt.figure(figsize=(23,11), layout="constrained")
    plt.xticks(rotation=90, ha='right', fontsize=16, fontweight="bold")
    # plt.yticks(ticks=np.arange(0.0, 1.01, 0.05), fontsize=17, fontweight="bold")
    
    # plt.ylim(0.0, 1.05)
    
    plt.title(f"Training Time of Systems, Trained on the Full and Subsets of the '{dataset_name.upper()}' dataset",\
               fontsize=27, fontweight="bold")
    plt.xlabel(f"Systems by the Classifiers and Regression Model-based Feature Selectors", fontsize=23, fontweight="bold")
    plt.ylabel("Systems Train Time per Record in Milliseconds", fontsize=23, fontweight="bold")
    ...
    delta_y = (y_max - y_min) / len(df['Test Time-Per Record'])
    
    plt.yticks(ticks=np.arange(0.0, y_max, 2*delta_y), fontsize=13, fontweight="bold", rotation=30)
    plt.ylim(0-y_min/2, y_max+delta_y)
    # addlabels(x=x, y=y-delta_y, ha="center", va="center", rotation=51)
    for i in range(len(x)):
        # print(f"delta_y:{delta_y}\ty[{i}]:{y[i]}\n")
        if y[i] <= 3*delta_y:
            plt.text(x=i, y=y[i]+3.7*delta_y, s=f"{round(y[i],1)}", ha="center", va="center", rotation=90, fontdict={"size":15, "weight":"bold"})
        else:
    ...
    if dataset_name == "nbaiot21":
        ...
        bar_color, plot_color, marker = ("lime", "lime", "o")

    plt.plot(x, y, color=plot_color, linewidth=3, linestyle="-", marker=marker, markersize=13, alpha=.751)
    plt.bar(height=y, x=x, color=bar_color, width=0.7, alpha=0.35)    

    # plt.grid(axis = 'y')
    plt.savefig(f"./TrainingTime-{dataset_name}.png", dpi = 100)
    plt.show()

def display_signle_record_test_time_metrics(df, fn):
    dataset_name = fn
    ...
    plt.title(f"Test Time of the Systems, Trained on the Full and Subsets of the '{dataset_name.upper()}' dataset",\
               fontsize=27, fontweight="bold")
    plt.xlabel(f"Systems by the Classifiers and Regression Model-based Feature Selectors", fontsize=23, fontweight="bold")
    plt.ylabel("Systems Test Time per Record in Milliseconds", fontsize=23, fontweight="bold")
    ...
    x = df[cols[0]] # 0 - MethodShortName
    y = df['Test Time-Per Record']
    y_max = max(df['Test Time-Per Record'])
    y_min = min(df['Test Time-Per Record'])
    delta_y = (y_max - y_min) / len(df['Test Time-Per Record'])
    
    plt.yticks(ticks=np.arange(0.0, y_max, 2*delta_y), fontsize=12, fontweight="bold", rotation=30)
    plt.ylim(0-y_min/3, y_max+delta_y)
    # addlabels(x=x, y=y-delta_y, ha="center", va="center", rotation=51)
    for i in range(len(x)):
        # print(f"delta_y:{delta_y}\ty[{i}]:{y[i]}\n")
        # if y[i] <= 2*delta_y:
        if x[i].startswith("CSVM"):
            plt.text(x=i, y=y[i] / 2, s=f"{y[i]}", ha="center", va="center", rotation=90, fontdict={"size":14, "weight":"bold"})
        else:
            plt.text(x=i, y=y[i]*2 + delta_y, s=f"{round(y[i],7)}", ha="center", va="center", rotation=90, fontdict={"size":14, "weight":"bold"})
    ...
    # plt.grid(axis = 'y')
    plt.savefig(f"./TestTimePerRecord-{dataset_name}.png", dpi = 100)
    plt.show()
...
def displayComparativeResultsByDatasets(mainDirPath):
    ...
    general_acc_results = {"System_Methods":ids_methods}
...
if __name__ == "__main__":
    ...
    display_entire_metrics(df_emr, file_name, eval_metrics="Precision")
    display_entire_metrics(df_emr, file_name, eval_metrics="ReCall")
    display_entire_metrics(df_emr, file_name, eval_metrics="F1Score")
    display_entire_metrics(df_emr, file_name, eval_metrics="Entire")
    ...
    # Displaying the test time per record values of the evaluation results.
    display_signle_record_test_time_metrics(df_emr, file_name)
